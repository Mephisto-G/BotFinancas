...
async function processarFila() {
    ...
    try {
        ...
        switch (acao) {
            case 'cadastrar': {
                const resultadoCadastro = cadastrarCompraParcelada(numeroLimpo, produto, valor, parcelas);
...
const PASTA_BACKUP = '/media/ps2share/backup';

function esperarFilaVazia() {
    return new Promise((resolve) => {
        const verificacao = setInterval(() => {
            if (filaDeMensagens.length === 0 && !botEstaOcupado) {
...
async function fazerBackupDoBanco() {
    console.log('[BACKUP] Aguardando a fila de mensagens esvaziar...');
    await esperarFilaVazia();

    try {
        if (!fs.existsSync(PASTA_BACKUP)) {
            fs.mkdirSync(PASTA_BACKUP, { recursive: true });
        }
        const nomeArquivo = `financeiro_${new Date().toISOString().slice(0, 10)}.db`;
        await db.backup(path.join(PASTA_BACKUP, nomeArquivo));
        console.log(`[BACKUP] ✅ Banco copiado para ${PASTA_BACKUP}`);
    } catch (erro) {
        console.error('[BACKUP] ❌ Falha ao fazer o backup:', erro.message);
    }
}
